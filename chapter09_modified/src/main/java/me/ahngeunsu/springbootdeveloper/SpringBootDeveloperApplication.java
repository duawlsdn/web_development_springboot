...
/*
    ...
    3. config/jwt/JwtProperties.java 생성 및 작성
    4. config/jwt/TokenProvider.java 생성 및 작성
    5.
 */
@EnableJpaAuditing
...
/*
    ...
    사용자가 서버에 접근할 때 이 사용자가 인증된 사용자인지 확인하는 방법 중 하나로,
        ...
        토큰 기반 인증 :
            * 토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
            ...
            서버가 토큰을 생성해서 클라이언트에게 제공하면, 클라이언트는 이 토큰을 가지고 있다가 여러 요청을
            해당 토큰과 함께 신청함. 이후 서버는 토큰을 확인하여 유효한 사용자인지를 검증하게 됨.
            ...
            토큰을 전달하고 인증 받는 과정
                ...
                4) 이후 인증이 필요한 API 를 사용할 때 토큰을 함께 보냄 ->
                5) 서버는 토큰이 유효한지 검증
            ...
            토큰 기반 인증의 특징
                1) 무상태성
                    ...
                    서버가 데이터를 유지하고 있으려면 그만큼 자원을 소비해야 하는데, 토큰 기반 인증에는 클라이언트 인증 정보가
                    담긴 토큰을 생성하고 인증하여 서버 입장에서는 클라이언트의 인증 정보를 저장하거나 유지할 필요 X
                    그래서 완전한 무상태(stateless)로 효율적인 검증 가능

                2) 확장성
                    무상태성과 관련하여, 서버를 확장할 때 상태 관리르 신경쓸 필요 X
                    세션 기반 인증은 각각 API 에서 인증을 해야하는 것과 달리 토큰 기반 인증은 토큰을 가지는 주체가 서버가 아닌
                    클라이언트이기 때문에 가지고 있는 하나의 토큰으로 결제 서버와 주문 서버에게 요청을 보내는 것이 가능
                    추가로 페이스북 로그인, 구글 로그인과 같이 토큰 기반 인증을 사용하는 다른 시스템에 접근해
                    로그인 방식을 확장할 수도 있고, 이를 활용해 다른 서비스에 권한을 공유할 수도 있음.

                3) 무결성
                    토큰 기반 인증 방식은 HMAC(Hash-based Message Authentication) 기법이라고도 하는데,
                    토큰을 발급한 이후에는 토큰 정보를 변경하는 행위를 할 수 없음.
                    만약, 토큰을 한 글자라도 변경하면 서버에서는 유효하지 않은 토큰이라고 판단

            JWT(JSON Web Token) -> json 형태로 주고받음
                발급 받은 JWT 를 이용해 인증하려면 HTTP 요청 헤더 중에
                Authorization 키 값에 Bearer + JWT 토큰 값을 넣어 보내야 함
                ...
                JWT 구조 : 온점(.)을 기준으로 헤더(header), 내용(payload), 서명(signature)로 이루어짐
                    ...
                    2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩러리를 클레임(claim) 이라고 하며,
                        키-값의 한쌍으로 이루어짐
                        등록된 클레임 / 공개 클레임 / 비공개 클레임 으로 나뉨
                        ...
                        구성 :
                            ...
                            exp : 토큰의 만료 시간(expiration).
                                시간은 NumericDate 형식으로(20250102),
                                항상 현재 시간 이후로 설정해야 함.
                            nbf : 토큰의 활성 날짜와 비슷한 개념으로 Not Before 를 의미함.
                                NumericDate 형식으로, 이 날짜가 지나기 전까지는 토큰이 처리되지 않음
                        ...
                        iss, iat, exp 는 JWT 자체에서 등록된 클레임
                        URI 로 네이밍된 https://ahngeunsu.com/jwt_claims/is_admin은 공개 클레임
    ...
    2. JWT 서비스 구현
        의존성과 토큰 제공자를 추가하고 리프레스 토큰 도메인과 토큰 필터를 구현 => JWT 서비스를 사용할 준비가 된 것.

        build.gradle 의존성 추가

        토큰 제공자 추가하기
            jwt 를 사용해서 JWT 를 생성하고 유효한 토큰인지 검증하는 클래스를 추가할 예정
            01 단계 - JWT 토큰을 만드려면 이슈 발급자(issuer), 비밀키(secret_key)를 필수로 설정해야 함.
                application.yml 을 수정합니다.

            수정 후에
            02 단계 - 해당 값들을 변수로 접근하는 데 사용할 JwtProperties 클래스를 만듭니다.
                config/jwt 패키지에 JwtProperties.java 파일을 만들어 코드를 작성.
