...
import me.ahngeunsu.springbootdeveloperapplication.config.jwt.JwtFactory;
...
class TokenProviderTest {
    ...
    // 2-2. invalidToken 검증 테스트
    @DisplayName("validToken() : 만료된 토큰일 때에 유효성 검증에 실패")
    @Test
    void validToken_invalidToken() {
        // given    -> 일부러 default 가 아니라 실패하는 token 을 생성하기 때문에 위의 메서드와 given 이 다름
...
/*
    1. generateToken()
        ...
        then : jjwt 라이브러리를 사용하여 토큰을 복호화
            -> 토큰을 만들 때 클레임으로 넣어둔 id 값이 given 절에서 만든 유저 ID 와 동일한지 확인

    2.
        1) validToken_validToken()
        given : jjwt 라이브러리를 사용, 토큰 생성. -> 만료 시간은 현재 시간으로 부터
            14일 뒤로 만료되지 않은 토큰으로 생성 -> Default 로 만들었습니다 -> JwtFactory.java

        when : 토큰 제공자(tokenProvider) validToken() 메서드를 호출해 유효한 토큰이지 검증한 결과값을
            boolean 값으로 반환

        then : 반환값이 true 인지 확인

        2) validToken_invalidToken()
        given : jjwt 라이브러리를 사용, 토큰 생성. -> 만료 시간은 1970년 1월 1일 부터 현재시간을 밀리초 단위로 치환한 값
            (new Date().getTime()) 에 1000을 빼서 이미 만료된 토큰으로 설정.

        when : 토큰 제공자(tokenProvider) validToken() 메서드를 호출해 유효한 토큰이지 검증한 결과값을
            boolean 값으로 반환

        then : 반환값이 false 인지 확인
    ...
    3. getAuthentication()
        ...
        then : 반환 받은 인증 객체의 유저이름(username) 을 가져와서 given 절에 초기화한
            subject 값인 user@gmail.com 과 같은 값인지 확인
    ...
    4. getUserId()
        ...
        then : 반환 받은 유저 ID 가 given 절에서 초기화한 1과 같은지를 확인
