...
class BlogApiControllerTest {
    ...
    /*
        ...
        : 이 클래스로 만든 자바 객체를 JSON 데이터로 변환하는 것 -> 직렬화
            ...
            형식에 맞게 변환하는 과정이 필요합니다. 이 작업을 직렬화/역직렬화
                ...
                1) 직렬화 Java 시스템 내부에서 사용되는 객체를 외부에서 사용하도록 변환하는 과정을 뜻하는데,
                예를 들어 title 은 "제목", content 는 "내용" 이라는 값이 들어가있는 개체가 있다고 가정했을 때,
                ...
                형태로 정리 됩니다.
        글 생성 API 를 테스트 하는 코드르 작성
        Given - 블로그 글 추가에 필요한 요청 객체를 생성
        When - 블로그 글 추가 API 에 요청을 보냅니다. 요청 타입은 JSON 이며, given 절에서 미리 만들어둔 객체를 요청 본문으로 함께 보냅니다.
        Then - 응답 코드가 201 Created 인지 확인
            Blog 를 조회 했을 때 전체 크기가 1인지 확인, 실제 저장된 데이터와 요청값을 비교
     */
    ...
    public void addArticle() throws Exception{
        ...
        List<Article> articles = blogRepository.findAll();                     // repository 에서 전체 글(객체)을 가져와 list 에 담음

        assertThat(articles.size()).isEqualTo(1);                      // list 내의 element 개수 확인해서 그게 1이면 true
        assertThat(articles.get(0).getTitle()).isEqualTo("제목");       // list 인 articles 를 확인해서 가장 첫번째(0번) 객체를 꺼내서, title 을 확인했더니 String "title"인지 확인
        assertThat(articles.get(0).getContent()).isEqualTo("내용");     // list 인 articles 를 확인해서 가장 첫번째(0번) 객체를 꺼내서, content 을 확인했더니 String "content"인지 확인

        /*
            List 에서 <> 제네릭 Article 클래스의 객체를 요소로 담게 됩니다,
            List 주요 메서드
                .get(인덱스 넘버) -> 0번째 인덱스에 저장된 Article 객체를 확인
            Article 객체의 필드는 title 과 content -> Getter 를 사용해서
            getTitle() 과 getContent()

            writeValueAsString() 메서드 : 객체를 JSON 으로 직렬화.
            그 이후에 MockMvc 를 사용해서 HTTP 메서드, URL, 요청본문, 요청 타입 등을 설정 뒤 테스트 요청 보냈습니다.
    ...
    /*
        삭제 구현
            ...
            BlogService 로 가서 delete() 메서드 추가 -> blogRepository 기준으로.
     */
    // 삭제 메서드
    ...
    public void deleteArticle() throws Exception{
        ...
        mockMvc.perform(delete(url, saveArticle.getId()))   // findArticle() 처럼 하나의 객체를 대상으로 할 때는 argument 가 url 만 있는게 아닙니다.
        ...
        // 가지고 온 List 에 아무런 값이 없는지 확인하는 단계 -> 왜 아무것도 없냐?
