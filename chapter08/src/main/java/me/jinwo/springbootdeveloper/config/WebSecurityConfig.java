package me.jinwo.springbootdeveloper.config;

import lombok.RequiredArgsConstructor;
import me.jinwo.springbootdeveloper.service.UserDetailService;
...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        ...
        return http
                ...
                .csrf(AbstractHttpConfigurer::disable)                      // 6. csrf 설정을 비활성화
                .build();
    }
...
/*
    1. 스프링 시큐리티의 모든 기능을 사용하지 않게 설정하는 코드
        ...
        h2 데이터를 확인하는데 사용하는 h2-console 하위 url 을 대상으로 .ignoring() 메서드
    ...
    2. 특정 HTTP 요청에 대한 웹 기반 보안을 구성. 이 메서드에서 인증 / 인가 및 로그인, 로그아웃 관련 설정
     ...
     3. 특정 경로에 대한 액세스 설정.
        ...
        permitAll() : 누구나 접근이 가능하게 설정.
            즉, "/login", "/signup", "/user"로 요청이 오면, 인증/인가 없이도 접근 가능
    ...
    8. 사용자 서비스를 설정
        userDetailService() : 사용자 정보를 가져올 서비스를 설정. 이 때 설정하는 서비스 클래스는 반드시
            USerDetailsService 를 상속받은 클래스여야 함.
        passwordEncoder() : 비밀번호를 인코더를 설정

    9. 패스워드 인코더를 빈으로 등록

    회원 가입 구현
        서비스 메서드 작성 -> 사용자 정보를 담고 있는 객체를 만들 수 있도록 해야합니다.
            -> dto / AddUserRequest.java 생성
 */
