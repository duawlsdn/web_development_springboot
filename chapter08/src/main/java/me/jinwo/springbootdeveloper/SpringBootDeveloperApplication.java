...
/*
    ...
    2. domain/User.java
    3. repository/UserRepository.interface
    4. service/UserDetailService.java
    5. springbootdeveloper 패키지 우클릭 -> new package -> config/WebSecurityConfig.java
...
/*
    학습 목표 : 로그인 / 회원 가입 기능을 구현
        ...
        /login 요청이 들어올 때,
        -> UserViewController 가 해당 요청에 대한 분기 처리를 하고 WebSecurityConfig 에 설정한 보안 관련 내용들을 실행
        -> UserDetailsService 를 실행하면 요청을 성공했을 때
        -> defaultSuccessUrl 로 설정한 /articles 로 리다이렉트 하거나 csrf 를 disable 하거나 하는 등의 작업

        UserDetailsService 에서는 loadUserByUsername() 메서드를 실행하여 이메일로 유저를 찾고 반환

        여기서 유저는 User 클래스의 객체이고 UserRepository 에서 실제 데이터를 가져 올겁니다.
        ...
        1. 스프링 시큐리티(Spring Security)
            ...
            2) 스프링 시큐리티
                : 스프링 기반 애플리케이션의 보안을 담당하는 스프링 하위 프레임워크 보안 관련 옵션을 다수 제공해줌
                CSRF, 세션 공격을 방어해주고(-> 정처기 등에 자주 나옴), 요청 헤더도 보안 처리를 해주므로 보안 관련 개발을
                하는데 부담을 덜어주는 편

            3) 필터 기반으로 동작하는 스프링 시큐리티
                스피링 시큐리티의 풀 로그인 과정을 설정하는 건(코드로 작성하는 건) 그렇게 어렵지 않음,
                내부적으로 아까 보여드린 그림처럼 꽤 복잡한 과정을 거치게 될것

                모두 외워야 하는 것 X, 다만 로그인 과정이 어떤 흐름으로 이어지는 지를 이해하면 더 잘 활용 가능
