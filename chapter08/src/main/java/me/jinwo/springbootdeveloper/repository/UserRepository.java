package me.jinwo.springbootdeveloper.repository;

import me.jinwo.springbootdeveloper.domain.User;
...
/*
    이메일로 사용자를 식별 가능, 즉 email 을 username 으로 봐도 됩니다. 따라서 사용자 정보를 가져오기 위해서는
    스프링 시큐리티가 이메일을 전달받아야 합니다. 스프링 데이터 JPA 는 메서드 규칙에 맞춰서 선언하면
    이름을 분석해 자동으로 쿼리(SQL문)를 생성해줍니다.

    findByEmail() 메서드는 실제 데이터베이스에 회원 정보를 요청할때 다음 쿼리를 실행
    ...
    🎈 findByEmail() 메서드가 요청하는 쿼리의 예시
        ...
        - WHERE age > ?1
        findByName(Is)Null() - "name" 컬럼의 값 중 null 인 데이터 반환
        - WHERE name IS NULL

        서비스 메서드 코드를 작성
        service 패키지에 UserDetailService.java 파일 생성
 */
