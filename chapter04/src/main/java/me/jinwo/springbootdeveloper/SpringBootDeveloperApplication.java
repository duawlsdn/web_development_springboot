...
/*
    1. 테스트 코드 개념 익히기
        테스트 코드는 작성한 코드가 의도대로 잘 작동하고 예상치 못한 문제가 없는지 확일할 목적으로 작성하는 코드
        보통 테스트 코드 관련 공부는 본 개발 공부를 하느라 미루는 경우가 많은데, 좀 익숙해지면 좋습니다!
        ...
        테스트 코드란?
            테스트 코드는 test 디렉토리에서 작업함. -> 프로젝트 생성시 src의 하위 폴터, main 과 같은 라인에 test 폴더 있음
            테스트 코드에는 다양한 패턴이 있는데, 일단 저는 given-when-then 패턴을 사용할 예정
                1) given : 테스트를 실행을 준비하는 단계
                2) when : 테스트를 진행하는 단계
                3) then : 테스트 결과를 검증하는 단계

            ex) 새로운 메뉴를 저장하는 코드를 테스트한다고 가정했을 때의 테스트 코드를 given,when,then 으로 구분해서 예시
            ...
            public void saveMenuTest(){
                ...
                // 3. then : 메뉴가 잘 추가 되었는지 검증
                final Menu savedMenu = menuService.findById(savedId).get();
                assertThat(savedMenu.getName().isEqualsTo(name));
                assertThat(savedMenu.getPrice().isEqualsTo(price));
            }

   이상의 코드를 확인하면 세 부분으로 나뉘어져 있음. 메뉴를 저장하기 위해 준비하는 과정인 given 절
   실제로 메뉴를 저장하는 when 절
   ...
   2. 스프링 부트 3와 테스트
        ...
        스프링 부트 스타터 테스트 목록(중 많이 쓰이는 것 두개만 소개)
            1) JUnit : 자바 프로그래밍 언어용 단위 테스트 프레임 워크
            2) AssertJ : 검증문인 어써션을 작성하는데 사용되는 라이브러리

            JUnit 이란?
                자바 언어를 위한 *단위 테스트 프레임 워크
                    *단위 테스트 : 작성한 코드가 의도대로 작동하는지 작은 단위로 검증함을 의미, 보통 그 단위는 메서드 기준
                JUnit 을 사용하면 단위 테스트의 결과가 직관적으로 나오는 편

                특징 :
                    1) 테스트 방식을 구분할 수 있는 애너테이션 제공
                    2) @Test 애너테이션을 메서드를 호출 할때 마다 새 인스턴스를 생성, 독립테스트가 가능
    ...
    AssertJ 로 검증문 가독성 높이기
        ...
        예를 들어서 아까 작성한 (JUnitTest.java 에서)
        Assertions.assertEquals(sum, a+b); 를 보시면 기대값과 비교값이 잘 구분되지 안흥ㅁ

        AssertJ 를 사용한 코드라인
        assertThat(a+b).isEquals(sum);
        이 경우 a 와 b 를 더한 값이 sum 과 같아야 한다는 의미로 읽힘 -> 영어의 경우
        ...
        테스트 코드 작성 연습 문제 풀어보기 -> test/java 폴더에 JUnitQuiz.java
