...
@AutoConfigureMockMvc   // MocMvc 생성 및 자동 구성 - 목업
class TestControllerTest {
    ...
    public void getAllMembers() throws Exception {   // throws Exception : 예외로 처리함
        // given
        final String url = "/test";     // 앞으로 자주 나오는 방식이라 알아두시길
        Member savedMember = memberRepository.save(new Member(1L,"홍길동"));
        // maybeags/web_development_java 에 c15_casting/ CentralControll.java/ Main.java 확인
...
/*
    ...
    (3) : andExpect() 메서드는 응답을 검증. TestController 에서 만든 API 는 응답으로 OK(200) 을 반환하므로 이에 해당하는 메서드인
        isOk() 를 사용해서 응답 코드가 200(OK) 인지 확인

    (4) : jsonPath("$[0].{필드명}")은 JSON 응답값의 값(value)을 가져 오는 역할을 하는 메서드.
        0(인데스)번째 배열에 들어있는 객체의 id,name 의 값을 가져오고 저장된 값과 같은지 확인
...
/*
    @SpringBootTest :
        애플리케이션 클래스에 추가하는 애너테이션인 @SpringBootApplication 이 있는 클래스를 찾고, 그 클래스에 포함돼 있는 빈을 찾은 다음,
        테스트용 애플리케이션 컨텍스트라는 것을 생성

    @AutoConfigureMockMvc
        MockMvc 를 생성하고 자동으로 구성하는 애너테이션으로, 이 것은 애플리케이션을 서버에 배포하지 않고도 테스트용  MVC 환경을 만들어 요청,
        전송, 응답 기능을 제공하는 유틸리티 클래스.
