...
/*
    Given
        - 테스트 유저를 생성, jjwt 라이브러리를 이용해 리프레시 톸큰을 만들어 데이터베이스에 저장.
        토큰 생성 API 의 요청 본문에 토큰을 포함하여 요청 객체를 생성
    When
        - 토큰 추가 API 에 요청을 보냅니다. 이때 요청 타입은 JSON 이며,
        given 절에서 미리 만들어둔 객체를 요청 본문으로 함께 보냅니다.
   Then
        - 응답 코드가 201 Created 인지 확인하고 응답으로 온 액세스 토큰이 비어 있지 않은지 확인
 */
...
/*
    chapter09
        토큰 기반 인증의 특징
        그 중에서 JWT 을 적용하고, 토큰 제공자(TokenProvider)를 만들었습니다.

        해당 인증 방법은 대중적으로 사용되는데다가 JWT 이 좀 현재 상황에서는 인기가 있는 편

        Token : 클라이언트를 구분하는데 사용하는 유일한 값
            서버에서 생성한 뒤에 클라이언트에게 제공한 뒤
            클라이언트는 서버에 요청 할 때마다 요청 내용과 함께 토큰 전송,
            서버가 하는 일은 토큰이 유효한 사용자인지 확인하는 것
            -> 세션 로그인 방식(form login)과의 차이

        JWT : 토큰 기반 인증 중 현재 인기 있는 방식 JSON 형식으로 클라이언트의 정보를 저장
            구성 :
       ...
       Refresh Token : 액세스 토큰과 별개의 토큰.
            액세스 토큰이 만료 되었을 때 새로운 액세스 토큰을 발급받는 용도
            (CreateAccessTokenRequest 로 RefreshToken 을 포함해서 요청했고,
            그 결과값으로 AccessToken 을 반환받는 구조로 구현
       ...
       Security Context : 인증 객체가 저장되는 보관소, 인증 정보가 필요할 때마다
            ...
            시큐리티 컨텍스트 객체를 저장하는 객체 -> Security Context Holder 였습니다.
