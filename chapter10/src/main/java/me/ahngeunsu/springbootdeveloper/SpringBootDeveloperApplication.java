...
/*
    사전 지식 : OAuth
        - 제 3의 서비스에 계정 관리를 맡기는 방식. naver/Google/kakao 로 로그인하기와 같은 예시
    ...
    OAuth 를 사용하면 인증 서버에서 발급받는 토큰을 사용해서 리소스서버에 리소스오너의 정보를 요청하고 응답 받아 사용 가능
        그렇다면 리소스오너의 정보를 어떻게 취득할 수 있는지에 대한 방법이 필요

        리소스 오너 정보를 취득하는 네 가지 방법
            1) 권한 부여 코드 승인 타입(Authorization Code Grant Type) :
                OAuth 2.0 에서 가장 대중적인 인증 방법. 클라이언트가 리소스에 접근하는데 사용
                권한에 접근 할 수 있는 코드와 리소스 오너에 대한 액세스 토큰을 발급받는 방식.
            ...
            권한 부여 코드 승인 타입이란?
                애플리케이션 / 리소스 오너(사용자) / 리소스 서버 / 인증 서버가 어떤 순서로 인증을 하는지 알 필요가 있음
    ...
    1. 권한 요청 :
        ...
        즉 카카오나 구글 권한 서버에 요청을 보내는 거라고 볼 수 있습니다. 요청 URI 는 권한 서버마다 다르지만
        보통은 클라이언트 ID, 리다이렉트 URI, 응답 타입 등을 파라미터로 보냅니다,
        실제 요청에 쓰이는 요청 URI 예사를 통해 주요 파라미터를 소개하자면,
        ...
        response_type : 클라이언트가 제공받길 원하는 응답 타입. 인증코드를 받을 때는 code 값을 포함해야 합니다.
    ...
    3. 인증 코드 발급
        사용자가 로그인에 성공하면 권한 요청 시에 파라미터로 보낸 redirect_uri 로 리다이렉션 됩니다.
    ...
    4. 액세스 토큰으로 발급
        ...
        grant_type : 권한 유형을 확인하는 데 사용. 이 때는 authorization_code 로 설정해야 함.
    ...
    5. 액세스 토큰으로 API 응답 & 반환
        ...
        🍎리소스 오너의 정보를 가져오기 위한 요청 예
            ...
            여기까지가 권한 부여 코드 승인 타입의 흐름입니다.
            대부분의 OAuth 를 구현한 라이브러리는 이 흐름을 바탕으로 코드를 구현하기 때문에
            흐름을 이해하고 넘어가는 것이 좋습니다.

        쿠키란?
            사용자가 어떤 웹사이트를 방문했을 때 해당 웹사이트의 서버에서 여러분의 '로컬 환경에 저장하는 작은 데이터'를 의미함.
            이 값이 있기 때문에 이전에 방문한 적이 있는지 알 수 있고, 이전에 로그인을 했다면 로그인 정보도 유지할 수 있는 것입니다.
    ...
    쿠키 관리 클래스를 구현
        ...
        01 단계 - springbootdeveloper /util / CookieUtil.java 파일을 생성한 뒤 코드를 입력하세요.
 ...
 2. 토큰 발급받기
        구글 로그인 기능을 추가하려면 인증 서버에게 토큰을 제공받아야 합니다.
            ...
            04 단계 - 사용자 인증 정보를 만들려면 OAuth 동의 화면을 먼저 구성해야 합니다.
                [동의 화명 구성] 버튼을 누르고 User Type 을 외부용으로 선택한 후 [만들기] 버튼을 클릭합니다.

            05 단계 - 값 입력 화면이 나타납니다. 앱 이름은 springboot-developer 로 입력하거나 자유롭게 입력하고, 이메일, 개발자 연락처를 꼭
    ...
    3. 스프링 시큐리티로 OAuth2를 구현하고 적용하기
        ...
        쿠키 관리 클래스 구현하기
            01 단계 - util 패키지를 새로 만들고 CookieUtil.java 파일을 생성한 뒤 코드를 입력하세요.
