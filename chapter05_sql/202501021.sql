...
-- 이전까지는 테이블 전체거나 혹은 컬럼에 관련된 부분들을 조회했지만
-- 현업에서는 데이터의 일부 컬럼을 가져오거나 상위 n의 데이터를 조회하는 것
-- 뿐만 아니라 특정 컬럼의 값이 A인 데이터만 가져오는 등 복잡한 쿼리를
-- 작성할 일이 있습니다.

-- 회원 정보 테이블 users에서 거주 국가(country)가 한국(Korea)인
-- 회원만 추출하는 쿼리문
...
-- WHERE 절에서는 여러 조건을 동시에 적용할 수 있고, 조건의 개수에 제한은 없음.
-- 논리연산 AND / OR / BETWEEN
	
-- 예제 : 회원 정보 테이블 uers에서 가입 일시(crated_at)가
-- 2010-12-01 부터 2011-01-01까지인 회원 정보를 출력해보세요
...
--			시간값을 조회할 때는 [컬럼명] BETWEEN [시작날짜] AND [종료날]
--			마찬가지로 시작날짜와 종료날짜를 포함한 모든 것을 출력해준다.

-- 예제 : 가입일시가 2010-12-01 부터 2011-01-01까지인 회원정보 출력
-- (BETWEEN을 사용하지 않고)
SELECT *
	FROM USERS
...
-- 응용 : users 에서 거주 국가가 Korea, Usa, UK 인 회원 정보를 추출하세요.
SELECT *
	FROM USERS
...
-- 응용 : users 에서 거주 국가가 Korea, Usa, UK가 아닌 회원 정보를 추출하세요.
SELECT *
	FROM USERS
...
-- LIKE : 해당 전치사 뒤의 작은따옴표/큰따옴표 내에서는 와일드카드를 사용가능
-- SQL을 해석하는 컴퓨터가 LIKE 코드를 읽는 순간 와일드카드를 감지하는데
-- SQL에서의 와일드 카드는 '%'로 0개 이상의 임의의 문자열을 의미하는
-- 메타문자(metacharacter)로 사용됨

-- 예제 users에서 country의 이름이 S로 시작하는 회원정보 추출
SELECT *
	FROM USERS
	WHERE COUNTRY LIKE "S%";
-- 거주국가가 S로 시작하는 (ex)South Korea) 모두를 출력
...
-- 응용 : users에서 conutry 이름이 S로 시작하지 않는 회원 정보만 추출
SELECT *
	FROM USERS
	WHERE COUNTRY NOT LIKE "S%";

-- IS : A IS B -> A는 B다 라는 뜻이기 때문에
-- A 컬럼의 값이 B 이다 일때 특히 null 일 때 '=' 대신 사용합니다.
-- 예제 : users에서 created_at 컬럼의 값이 null 인 값 출력
SELECT *
	FROM USERS
...
-- 연습문제
	-- 1. users에서 country 가 Mexico 인 회원 정보 추출
...
SELECT *, 			-- '전부 다'의 기준은 기존 컬럼
	(PRICE - DISCOUNT_PRICE) as "dicount_amount"
	FROM PRODUCTS
...
SELECT id, name, price
	...
	-- 5. orders에서 order_date 가 2015-07-01부터 2015-10-31까지가
		-- 아닌 정보만 출력할 것
SELECT *
	FROM ORDERS
...
-- SELECT *, 			-- '전부 다'의 기준은 기존 컬럼
-- 	(PRICE - DISCOUNT_PRICE) as "dicount_amount" -- 새로운 컬럼
-- 5. NOT BETWEEN 순인 것만 기억
...
-- 정렬 할 때 사용 : 숫자의 경우는 쉽게 알 수 있지만, 문자의 경우는
-- 아스키 코드(ASCII Code)를 기준으로 합니다.
	
-- 예제 : users에서 city를 기준으로 내림차순 하여 전체 정보 출력
SELECT *
	FROM USERS
	ORDER BY CITY DESC;

-- 예제 : users에서 created_at를 기준으로 오름차순 하여 전체 정보 출력
SELECT *
	FROM USERS
...
-- ORDER BY의 특징 : 컬럼명 대신에 컬럼 순서를 기준으로 하여 정렬이 가능.
	-- 컬럼며을 몰라도 정렬할 수 있다는 장점이 있지만 주의할 필요가 있음

-- 예제 : users에서 username, phone, city, country, id 컬럼을 순서대로 출력
-- 하는데, 첫번째 컬럼 기준으로 오른차순 정렬해보세요.
SELECT username, phone, city, country, id
	from USERS
	order by 1 asc;
-- 아까와 같이 ORDER BY 1 ASC 로 했짐나 정렬이 id 가 아닌 username을 기준으로
-- ASCII Code가 적용되어 정렬 방식이 달라졌음을 확인 가능.
-- 즉, ORDER BY는 데이터 추출이 끝난 후를 기점으로 적용이 된다는 점을 명심

-- 예제 : users에서 city, id 컬럼만 출력하는데, city 기분 내림차순
--	id 기준 오름차순으로 정렬
SELECT city,id 
	from USERS
...
-- 3. orderderails에서 product_id를 기준 내림차순, 같은 제품 아이디 내에서는
-- quantity 기준 오름차순으로 모든 컬럼 출력
SELECT *
	FROM ORDERDETAILS
	order by PRODUCT_ID desc, QUANTITY asc;	-- asc 적어도 되고 안적어도 됩니다. default = asc
	
-- 여태까지 배운 것을 기준으로 실무에서는 어떤 방식으로 데이터 추출해서 사용하는지 예시
-- 실무에서는 select, where, order by를 사용해서 다양한 데이터를 추출하는데,
...
SELECT *
	from 음식점정보
	WHERE 지역 = "마포구"
		AND 1인분가능여부 = 1;		-- 1을 쓸 경우에는 true 의미 / 0은 false 의미
