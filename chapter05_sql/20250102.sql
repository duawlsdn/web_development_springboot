...
-- 결과 창의 첫 행에 각각 Find, Insight, With SQL 을 3개의 칸에 
...
select *
	from users;
-- users.csv 파일에 있었던 모든 테이블과 컬럼과 값이 출력이 됐음을 확인 간능
...
-- 얘가 지 혼자서 소문자로 바뀌어가지고 미리 설명드리는데
-- SQL 대소문자를 구분하지 않습니다.
-- LIMIT : 갯수 제한을 거는 명령어(IDE를 쓰느냐에 따라서 TOP일 때도 있습니다.)
SELECT * from products limit 3;
-- 이렇게 *를 사용해서 전체 정보를 조회하는 것을 full scan을 쓰는데, 빈도가 높지 않습니다.
-- 물론 시험에도 자주 나오지 않습니다.

-- 갯수 제한을 걸기 위한 LIMIT와 특정 컬럼을 조회하는 형태로 수업을 하도록 하겠습니다.
