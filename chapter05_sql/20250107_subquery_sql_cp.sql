...
-- users에서 country가 Korea 인 회원 정보만 추출하고(1번 추출),
-- Mexico인 회원 정보만 추출해서(2번 추출)
-- (단, 컬름은 id, phone, city, country만 출력하고, 최종 결과 집합은 country 기준 알파벳 순)
...
-- 두 결과 집합을 union all 을 사용해서 하나로 결합하세요.(단, 최종 결과는 최신순으로 정렬)
-- 1번 방식
...
-- YYYY-MM-DD 형식으로 지정돼있다면, 문자열 비교 결과와 실제 날짜 비교 결과가 동일하게 적용됨
-- 그래서 MM-DD-YYYY 형태로 돼있다면, 오루가 발생할 가능성이 있습니다.

-- 2. users에서 USA에 거주 중이면서 마케팅 수신에 동의(1)한 회원 정보와 France에 거주 중이면서
-- 마케팅 수신에 동의하지 않은(0) 회원 정보를 select 로 각각 추출하고, 두 결과 집합을 UNION ALL을
-- 사용해서 하나로 결합하라(단, 최종결과는 id,phone,country, city, is_marketing_agree
...
-- 3. UNION을 활용하여 orderdetails와 products를 FULL OUTER JOIN 조건으로 결합하여 출력하세요. -> 시험문제 or 실무에까지 이용하는 경우 거의 없음.
...
-- SELECT AVG(price) FROM products; 를 할 경우 전체 price / 행의 갯수로 나눈 데이터가
-- 단 하나이므로 SELECT name, price, AVG(price) from products; 로 작성하면
-- 1행 짜리만 도출됨

-- products 테이블의 name/ price 를 불러오는 것은 기본적인 select 절입니다.
-- 그런데 select 절에는 단일값을 반환하는 서브 쿼리가 올 수 있습니다.

-- 스칼라(Scalar) 서브 쿼리 : 쿼리의 결과가 단일 값을 반환하는 서브 쿼리

SELECT name, price, 38.455 AS avgPrc FROM products;

-- 특정한 단일 결과값을 각 행에 적용하고 싶다면 이상과 같은 하드코딩이 가능합니다.
-- 하지만 정확한 값을 얻기 위해서 사전에 쿼리문으로 
-- select AVG(price) from PRODUCTS; 가 요구 된다는 점에서 효율적이지 않고,
...
-- users 에서 city 별 회원 수를 카운트하고, 회원 수가 3명 이상인 도시명과 회원수를 출력해보자
-- (단, 회원수를 기준으로 내림차순)
SELECT city, COUNT(distinct id) from users group by city;	-- > 도시별로 id 개수를 계산 했습니다.
...
-- orders와 staff를 활용해서 last_name 이 Kyle 이나 Scott 인 직원의 담당 주문을 출력할려면?
-- (단, 서브쿼리 형태를 활용하자)
SELECT id
	FROM staff
	WHERE last_name IN ("Kyle","Scott")		
	-- > 조건절에 쓰일 경우에 scalar 서브가 아니었다는 점에 주목
	;
...
--  컬럼 개수와 필터링 적용 대상 컬럼으 개수가 '일치'해야만 합니다.
...
-- 결과 값으로 직원 정보 테이블에 존재하는 id, user_id 와 동일한 값이 
-- orders 테이블의 staff_id, user_id 컬럼에 있을 경우 반환하여 출력합니다.
...
-- 주문 상제 정보 테이블 orderdetails에서 quantity가 50 이상인 정보를 각각 서브 쿼리로 작성하고,
-- INNER JOIN 하여 출력
...
-- 1)과 2)의 INNER JOIN 구문 -> FROM 절에서 이루어짐
SELECT *
	FROM 
		(SELECT *
			FROM orders 
...
-- ex) scalar인 경우에는 select 절에 들어가는 것처럼
...
-- 여러 개의 컬럼을 추가하고 싷을 때는 서브 쿼리를 여러개 작성하면 됩니다.
...
-- FROM에서 사용되는 서브 쿼리 : '인라인 뷰', 마치 테이블 처럼 서브 쿼리의 결과값을 사용 가능
...
-- 인데스(index) : 테이블의 검색 속도를 높이는 기능으로, 컬럼 값을 정렬하여 검색시 더욱 빠르게
-- 찾아내도록 하는 자료 구조
...
-- 특징 : IN 연산의 경우에 다중 컬럼 비교를 할 때는 서브 쿼리에서 추출하는 컬럼으 ㅣ개수와
...
-- 1. users 에서 created_at 컬럼 활용하여 연도별 가입 회원 수를 추출
SELECT *
	FROM users ;
...
-- 2. users 에서 country, city, is_auth 컬럼을 활용, 국가별, 도시별로 본인 인증한 회원 수를 추출
SELECT country, city, SUM(is_auth)		-- > is_auth가 1이면 본인인증한거니까 is_auth들의 합을 구함
	from users 
