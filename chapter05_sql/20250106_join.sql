...
-- 1번 쿼리
SELECT SUBSTR(CREATED_AT , 1, 7) as month ,COUNT(distinct id) as uniqueUesrCnt
	from USERS
...
-- 표준 SQL에서는 1번 쿼리만 가능, MariaDB에서는 ALIAS를 Group by, Order by에 사용 가능(2번 쿼리가 MariaDB에서는 가능)
-- SQLD / P 에서는 2번 쿼리와 같은 방식으로 출제 되지 않음.
-- DB간 호환성을 염두에 두고 있을 때는 1번 쿼리 방식으로 장식하는 것이 안전합니다.
...
-- 여러 테이블을 하나의 FROM에서 다룰 때에는 별칭을 사용 가능(ALISA와는 다릅니다 -> 컬럼명을 지정)
-- FROM uers u 로 작성했을 때 이후에는 u만 썼을 경우 users 테이블을 의미하게 됨
-- 그래서 이후에는 FROM 절에서 다수의 테이블명을 기입하게 될 경우에 별칭을 통해서 정리하여 SQL문을 효율적으로 사용할 수 있게 됨.

-- 이상의 문제에서의 조건은 '주문정보가 있는 회원의 정보만 출력' 하는 것이므로, orders 내에
-- user_id가 일부 기분이 되어야 합니다.
...
-- -> 이미 JOIN을 통해 하나의 테이블로 구성된 것처럼 간주 되기 때문.

-- 복수의 테이블이 하나로 결합되기 위해서는 두 테이블 간에 공통괸 부분이 존재해야 합니다.
-- RDBS 에서는 이 부분을 키(Key)라고 합니다.

-- 키 값은 테이블에 반드시 1개 이상 존재하도록 설계되어 있고(여러분이 설계 해야 하고)
-- 테이블에서 개별 행을 유일하게 구분 짓습니다. 따라서 키값은 컬럼 내에서 중복되지 않으며
...
-- 하지만 여러 테이블을 조인하며 '키 값도 중복될 수 있다' 예를 들어 회원아이디가 7인 사람이
-- 세번 주문했다면 회원 정보(users)와 주문정보(orders)를 결합한 결과에는 u.id = 7인
-- 행에 3개 있을 것이다. 이 경우 '한 번 이라도 주문한 회원 수'를 중복 없이 구하려면 회원
...
-- PK 는 다른 테이블의 고유한 키 값인 PK를 참조 한다.(orders에서 o.id가 FK에 해당)
-- users의 u.id가 PK에 해당해서 FK가 PK를 참조해서 조건을 합치시켜 JOIN 함.)

-- 예를 들어서 OK값이 A,B,C 만 있다면 PK 값도 이값만 가질 수 있고, 또한 중복되지 않는다는 특징을 지니고 있음.
-- 하지만 FK의 경우에는 참조하고 있는 관계에 따라 참조 대상인 PK값이 여러 번 나타날 수 있습니다.
-- users 테이블에서는 id = 7인 값이 하나만 존재하지만
-- orders 테이블에서는 uers_id 가 PK가 아니기 때문에 3건 존재합니다.
-- 이를 JOIN 시켰을 때 u.id로 ORDER BY하더라도 합친 테이블 상에서 제 1컬럼(즉 PK 컬럼)에
-- 동일 id가 여러 번 출력 될 수 있습니다.

-- JOIN의 종류
-- 1. INNER JOIN(교집합)
--  : 두 테이블의 키 값이 일치하는 행의 정보만 가지고 옵니다. -> 집합에서 '교집합'에 해당
-- users와 orders를 하나로 결합하여 출력합니다.(단 주문 정보가 있는 회원의 정보만 출력)
-- 이상의 문제에서 INNER JOIN을 도출할 수 있는 근거는 '(단 주문 정보가 있는 회원의 정보만 출력)' 이 부분에 해당함.
...
-- INNER JOIN 의 경우 두 테이블간의 키 값 조건이 일치하는 행만 결과값으로 가져왔습니다.(교집합만)
...
-- 즉, 드 테이블의 교집합과 교집합에 속하지 않는 왼쪽 차집합을 불러옵니다.
...
-- 회원의 정보까지 출력하려면 LEFT JOIN을 활용핣니다. (제가 든 예시는 블로그 글을 남긴 user와)
-- (한 번도 남기지 않는 user를 구분할 때였습니다.)

-- '결합 후에' 컬럼 값에 접근할 때는 [테이블 별칭].[컬럼명]으로 내부 컬럼에 접근합니다.
-- 두 테이블에 동일한 컬럼이 있을 때(대부분의 경우 PK 역할을 하는 id 들은 다 있으니까요),
-- 어떤 것을 지정했는지 명확히 하기 위해서 입니다.
-- 이를 활용해서 SELECT에서도 * 대신에 표시할 컬럼을 지정 할 수 있는데,
-- u.id, u.username, o.order_date 처럼 컬럼이 속한 테이블 별칭을 . 앞에 명시해주면 됩니다.
...
-- users 와 orders를 하나로 결합해 출력해봅시다.(단, 주문정보가 없는 회원의 정보만 출력합니다.)
SELECT *
	FROM users u left join orders o on u.id = o.user_id 
	WHERE o.id IS NULL -- o.id / o.user_id / o.staff_id / o.order_date
	;

-- users 와 orders를 하나로 결합하고, 추가로 orderdetails에 있는 데이터도 출력해 보자.(단,
-- 주문 정보가 없는 회원의 주문 정보도 모두 출력하고, 다음 컬럼을 출력해보자. u.id, u.username,
-- u.phone, o.user_id, o.id, od.order_id, od.product_id) -> JOIN이 두번 일어납니다.

-- FROM 내에서는 JOIN을 중첩해서 횟수 제한 없이 사용 가능
-- 첫번째 JOIN의 ON 뒤에, 두번째 JOIN 절을 작성하면 됩니다.
...
-- users 와 orders를 하나로 결합하여 출력해보자(단, 회원 정보가 없는 주문 정보도 출력).
SELECT *
	FROM users u right join orders o on u.id = o.user_id;

-- RIGHT JOIN은 기본적으로 LEFT JOIN 과 기능은 동일합니다. LEFT JOIN 에서는 왼쪽에 위치한
...
-- 이는 두 테이블간의 '포함관계' 에서 비롯됩니다.
-- users테이블에 id가 없는 회원의 정보는 orders테이블에 존재 할 수 없음.
-- 따라서 orders 테이블간의 user_id 컬럼은 모두 users 테이블의 id 컬럼에 있는 값에 해당합니다.
-- 결합으로 봤을 때 orders는 users 테이블에 종속돼있으므로 null 값이 출력될 수 없습니다.

-- 이상을 이유로,
-- 많은 기업에서는 RIGHT JOIN 대신에 LEFT JOIN을 사용하도록 권장합니다.

-- users 와 orders 의 '모든 가능한 행 조합'을 만들어 내는 쿼리를 작성합니다.
SELECT *
...
-- u.id 와 o.user_id 를 연결하는 등의 조건 없이 두 테이블의 모든 행을 합쳐서 만들 수 있는 모든 경우의 수를 생성한 것입니다.
...
-- 실제 환경에서는 CROSS JOIN 을 제한하는 편입니다. 컴퓨터에 많은 연산을 요구하는데,
-- 실질적으로는 NULL 값만 출력되는 경우의 수가 많아 쓸모는 없기 때문입니다.

-- 연습 문제
-- 1. users 와 staff를 참고하여 회원 중 직원인 사람의
-- 회원 id, 이메일, 거주 도시, 거주 국가, 성, 이름을 한 화면에 출력하세요.
SELECT u.id, u.username, u.country, s.first_name, s.last_name
	FROM users u inner join staff s on u.id = s.user_id
	;

-- 2. staff 와 orders를 참고하여 직원아이디가 3번 5번인 직원의 담당 주문을 출력하세요.
-- (단, 직원 아이디, 직원 성, 주문 아이디, 주문일자만 출력하세요.) -> WHERE절 사용 
...
-- 4. orders 와 orderdetails, products를 참고하여 회원 아이디별 주문 금액의 총합을 정상 가격과
...
-- JOIN 총 정리
-- JOIN : 두 테이블을 하나로 결합 할 때 사용(정규화를 통해서 DB 관리를 효율화 했기 때문에 하나로 합치게 될때 사용되는 명령어)

-- 정리 1. 기본형식
-- FROM [테이블명1] a(별칭) [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명2] b(별칭)
-- ON [JOIN조건]		-> 선호되는 조건 방식 PK = FK
...
-- 키 값은 여러개가 있을 수 있어, 어떤 값을 지누으로 할 지 ON에서 명시합니다.
...
-- FROM 내에서 JOIN을 여러 번 중첩 사용 가능. 앞의 조인의 ON 절 뒤에 새로운 JOIN 명령어를
...
-- 가장 기본적인 JOIN 문으로, 간혹 INNER를 생략하고 JOIN만 쓰기도 합니다.(default JOIN 구문)
...
-- CROSS JOIN은 두 테이블을 결합했을 때 각 테이블의 행으로 만들 수 있는 모든 조합을 결과값으로 도축하는 연산
-- 카테시안 곱이라고 함. -> 쓸 때 주의

-- 주의사항 : FULL OUTER JOIN vs CROSS JOIN
-- FULL OUTER JOIN의 경우 ON 조건에 부합할 때만 결과값을 도출하는 반면에
-- CROSS JOIN은 모든 경우의 수를 출력하기 때문에 ON조건을 명시하는 일이 거의 없다.

-- 강사 정리 : 실무에서는 INNER JOIN(default), LEFT OUTER JOIN을 기본으로 사용
--			OUTER를 명시하는 회사도 있음.