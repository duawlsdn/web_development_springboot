...
-- 예를 들어 실습 데이터에서 회원 수를 구하려면 어떻게 해야할까, 전체 회원 수가 아니라,
...
-- 이럴 경우 사용하느 명령어가 GROUP BY
-- 이를 이용해 데이털르 그룹으로 묶은 후 필요 함수 적용 가능
-- 예를 들어 '국가별 회원 수를 구하라' 혹은 '일별 매출 계산하라'와 같은
-- 그룹화 기준을 지정해서 원하는 계산을 수행 할 수 있음.

-- GROUP BY로 계산한 결과를 필터링하는 HAVING 명령어
-- GROUP BY를 적용해서 '국가별 회원 수를 구하라' 이후에 '그리고 회원 수가 10명 이상인'
...
-- country의 데이터값의 종류를 계산하고 싶다면 어떻게 해야할까?
SELECT count(distinct country)	-- 중복을 제거하기 위해서는 distinct
	from USERS;

-- DISTINCT : 중복값을 제거하여 고유한 값만 반환하는 키워드 / SELECT와 함께 사용
...
-- products에서 최대가를 구하시오
SELECT MAX(price)
	from PRODUCTS;

-- products에서 전체 데이터 가격의 합 구하시오
SELECT SUM(price)
	from PRODUCTS;

-- products에서 price의 평균(단, 결과값의 컬럼명을 avgPrice로 변경)
SELECT ROUND(AVG(PRICE),2)	-- ROUND(집계함수결과, 소수점몇째자리까지인지)
	as "avgPrice"
	FROM PRODUCTS;
...
SUBSTR(컬럼명, 시작위치, 가져올문자개수) : 문자열을 지정한 시작 위치부터 지정한 문자 개수만 -> 
LENGTH(컬럼명) : 문자열의 길이 반환
UPPER(컬럼명) : 알파벳 문자열을 대문자로 반환
LOWER(컬럼명) : 알파벳 문자열을 소문자로 반환

집계 함수는 여러 행의 데이터를 '하나의 결과값'으로 집계하는 반면, 일반 함수는 한 행의 데이터에 하나의 결과값을 반환
...
group by
	...
	-- 집계 함수만으로 원하는 결과를 얻을 수 없을 때(데이터 전체가 아니라, 원하는 기준으로 그룹을 나눈 상태에서 계산을 해야할 때를 의미)가 있습니다.
	-- 예를 들어, 전체 회원 수가 아닌 국가별 회원 수를 나누어 계산하기도 하며, 월별로 가입한 회원 수를 집계하기도 하기 때문
	-- 즉, 특정 조건으로 나눈 그룹별 계산을 수행할 때는 먼저 그룹화를 선행해야 합니다.
	-- 그 후 집계 함수는 특정 컬럼을 기준으로 데이터를 그룹화 한 후에, 그룹별로 집계 함수를 적용해야 합니다.
...
-- SQL문을 작성해야 합니다.
-- 국가 수는 적지도 않고, 실제 환경에서는 훨씬 더 많은 데이터를 다루는데, 조건을 일일이 적용하기에는 비효율적
-- 즉, where절에서 country =의 값을 매번 바꾸는 방식은 사용하지 않음

-- 다음 시간에 저희는 GROUP BY를 통해 어떻게 묶을지 학습