...
SELECT country, Count(distinct id) as uniqueUseCnt
	from USERS
	group by country;
-- --------- > 뒤에 나오는 컬럼명을 기준을 그룹화 해서
-- country를 표시하고, COUNT() 적용한 컬럼도 표시해서 조회해줘

SELECT * from USERS;

-- users에서 country가 Korea인 회원 중에서 마케팅 수신에 동의한 회원 수를 구해 출력할 것
-- 표시 컬럼 country, uniqueUesrCnt 두 컬럼으로 구성돼있습니다.(GROUP BY - X)
SELECT country, COUNT(distinct id) as uniqueUesrCnt
	from USERS
	WHERE country = "Korea" and IS_MARKETING_AGREE = 1;

-- users에서 country 별로 마케팅 수신 동의한 수와 동의하지 않은 회원 수를 구해 출력
-- 표시 컬럼 country, uniqueUesrCnt(GROUP BY - O)
SELECT country, IS_MARKETING_AGREE, COUNT(distinct id) as uniqueUesrCnt
	from USERS
	group by country, IS_MARKETING_AGREE
	order by country, uniqueUesrCnt desc;
-- 		국가별로는 오름차순, uniqueUesrCnt를 기준으로 내림차순
...
-- 2) GROUP BY 뒤에는 그룹화할 컬럼명을 입력. 그룹화한 컬럼에 집계 함수 적용
		-- 그룹별 계산을 수행
-- 3) 형식 : GROUP BY 컬럼명1, 컬럼명2, ...
-- 4) GROUP BY에서 두개이상의 기준 컬럼을 조건으로 추가하여 여러 그룹으로 분할 가능한데,
--     컬럼 순서에 따라 결과에 영향을 미치므로 우선순위를 생각할 필요 있음

-- user에서 국가(country) 내 도시(city)별 회원수를 구하여 출력,.
-- 단, 국가명은 알파벳 순서대로 정렬, 같은 국가 내에서는 회원 수 기준으로 내림 차순 정렬,
-- 표시컬럼 country, city, uniqueUesrCnt(WHERE절 - X)
SELECT country, city, COUNT(distinct id) as uniqueUesrCnt
	from USERS
	group by country, city
	order by country asc, uniqueUesrCnt desc;

-- SUBSTR(컬럼명, 시작값, 글자개수)
-- users에서 월별(ex. 2013-10) 가입 회원 수를 출력할 것
-- 가입일시 컬럼 활용하고, 최신순으로 정렬할것.
SELECT SUBSTR(CREATED_AT , 1, 7) as month ,COUNT(distinct id) as uniqueUesrCnt
	from USERS
	group by substr(CREATED_AT,1,7)
	order by month DESC;

-- 1. orderdetails에서 order_id 별 주문 수량 quantity의 총합을 출력할것
	-- 주문 수량의 총합이 내립차순으로 정렬되도록 할것
...
-- 2. orders에서 staff_id 별, user_id 별로 주문 건수(COUNT(*))를 출력할 것
	-- 단, staff_id 기준 오름차순하고 주문건수 별 기준 내림차순
SELECT staff_id, user_id , COUNT(*)
	from ORDERS
	group by STAFF_ID , USER_ID
	order by STAFF_ID , count(*) desc;

-- 3. orders에서 월별로 주문한 회원수 출력할 것(order_date 컬럼 이용, 최신순으로 정렬)
SELECT SUBSTR(ORDER_DATE , 1, 7) ,COUNT(distinct id)
	from ORDERS
...
-- 예를 들어서, 회원 수가 n명 이상인 국가의 회원 수만 보는 등의 조건을 걸려면 어떻게 해야할까?

-- WHERE절 이용하는 방법이 있긴 하지만 추가적인 개념에 대해서 학습할 예정
...
만약에 회원 수가 8명 이상인 국가의 회원 수만 필터링하려면 어떻게 할까?
-- SELECT country, count(distinct id)
-- 	from USERS
-- 	WHERE count(distinct id) >= 8;
--	오류 나는 사례

-- users에서 회원 수가 8명 이상인 country 별 회원 수 출력(회원 수 기준 내림차순)
...
1. where에서 필터링을 시도할 때 오류가 발생하는 이유 :
	...
	하지만 집계 함수로 계산ㄴ된 값의 경우에는 그룹화 이후에 이루어지기 때문에
...
-- 주문 회원 수가 40명 이하인 데이터만 출력(단, 주문 건수 기준으로 내림차순 정렬할것.
...
-- HAVING 정리
-- 순서상 GROUP BY 뒤쪽에 위치하며, GROUP BY와 집계함수로 그룹별로 집계한 값을 필터링 할 때 사용
...
-- 1. 컴퓨터는 가장 먼저 FROM을 읽어 데이터가 저장된 위치에 접근하고, 테이블의 존재 유무를 따지고
-- 테이블을 확인하는 작업을 실행하고,
-- 2. WHERE 을 실행하여 가져올 데이터의 범위 확인
-- 3. 데이터베이스에서 가져올 범위가 결정된 데이터에 한하여 집계 연산을 적용할 수 있게 그룹으로 데이터를 나눈다.
-- 4. HAVING 은 바로 그 다음 실행되면서 이미 GROUP BY 를 통해 집계 연산 적용이 가능한 상태이기 때문에
-- 	4의 단계에서 집계 연산을 수행함
-- 5. 이후 SELECT 를 통해 특정 컬럼, 혹은 집계 함수 적용 컬럼을 조회하여 값을 보여주는데,
-- 6. ORDER BY를 통해 특정 컬럼 및 연산결과를 통한 오름차순 및 내림차순으로 나열함.

-- 연습 문제
-- 1. orders에서 회원별 주문 건수 추출하라(단 주문건수가 7건 이상인 회원의 정보만 추출,
...
-- 3. users에서 USA, Brazil, Korea, Argentina, Mexico에 거주 중인 회원 수를 국가별로 추출
-- 	(단, 회원 수가 5명 이상인 국가만 추출하고, 회원 수 기준으로 내림차순)
-- 	(힌트 : where도 있고 Having도 있습니다.)
SELECT COUNTRY, COUNT(distinct id)
	from USERS
