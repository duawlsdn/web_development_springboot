...
/*
    1. 스프링 부트 3 구조 살펴보기
        ...
        계층 : 각자의 역할과 책임이 있는 어떤 소프트웨어의 구성 요소
            -> 각 계층은 소통할 수 있지만 다른 계층에 직접 간섭하거나 영향을 미치진 않음

        ex) 카페와 빵집
            카페에서는 커피를 팔고, 빵집에서는 빵을 파는데, 필요한 경우 서로 협업 관계를 맺어 어떤 손님이 커피를 사면 빵을
            할인해 줄 수는 있음 -> 계층 간 소통
            그런데 빵집 점원이 커피를 서빙해 줄 수는 없음

        스프링 부트에서의 계층
            1) 프레젠테이션(Presentation)
                HTTP 요청을 받고 이 요청을 비지니스 계층으로 전송하는 역할 -> Controller 가 프레젠테이션 계층 역할
                TestController 클래스와 같은 것을 의미하며 스프링 부트 내에 여러 개가 있을 수 있음
            2) 비지니스(Business)
                모든 비지니스 로직을 처리
                    * 비지니스 로직 : 서비스를 만들기 위한 로직을 의미하며, 웹사이트 에서 벌어지는 모든 작업 ->
                    주문 서비스라고 한다면 주문 개수, 가격 등의 데이터를 처리하기 위한 로직, 주문 처리를 하다가 발생하는 예외 처리 로직,
                    주문을 받거나 취소하는 등 프로세스를 구현하기 위한 로직등을 의미
                서비스가 비지니스 계층의 역할을 함
            3) 퍼시스턴스(Persistence)
                모든 데이터베이스 관련 로직을 처리, 이 과정에서 데이터베이스에 접근하는 DAO(추후 수업 예정) 객체를 사용할 '수도' 있음
                    * DAO - 데이터베이스 계층과 상호작용하기 위한 객체라고 이해하면 편함
                    -> Repository 가 퍼시스턴스 계층의 역할을 함
            계층은 개념의 영역이고 컨트롤러, 서비스, Repository 는 실제 구현을 위한 영역

      스프링 부트 프로젝트 디렉토리 구성
        : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발자들이 따르듯이 수업시에도 해당 구조를 따라 개발 예정

            main : 실제 코드를 작성하는 공간, 프로젝트 실행시에 필요한 소슷 코드나 리소스 파일은 모두 이 폴더 안에 들어있음(프로젝트 생성시 자동 구현)
            ...
            지시사항
            main 디렉토리 구성하기
                : main 디렉토리 내에 java 와 resources 로 구성돼 있음. 여기에 아직 추가하지 못했던 스프링 부트 프로젝트의 구성요소 추가
            ...
            03 단계 - 스프링부트 설정을 할 수 있는 application.yml 파일을 생성
                -> 해당 파일은 스프링 부트 서버가 실행되면 자동으로 로딩되는 파일로, 데이터베이스의 설정 저보,로깅 설정 정보 등이 들어가 수도 있고,
                직접 정의 할 때 사용하기도 함. 앞으로 프로젝트를 진행하며 자주 사용할 파일
                resources 우클릭 -> new file -> application.yml

    2. 스프링 부트 3 프로젝트 발전시키기

        각 계층의 코드를 추가할 예정 -> 이를 통해 계층이 무엇이고, 스프링 부트에서는 계층을 어떻게 나누는지를 감을 잡아나가실 수 있도록 수업
        지금은 의존성을 추가한 다음에 -> 프레젠테이션 계층, 비지니스 계층, 퍼시스턴스 계층 순서로 코드를 추가할 예정
        ...
        testImplementation : 프로젝트의 테스트 모드를 컴파일하고 실행할 때만 필요한 의존성 설정,
            테스트 코드에서만 사용, 메인 애플리케이션에는 사용하지 않음

        runtimeOnly : 런타임에만 필요한 의존성을 지정, 컴파일 시에는 필요하지 않지만, 애플리케이션을 실행 할 때 필요한 라이브러리 설정
