...
public class ExampleController {
    ...
    public String thymeleafExample(Model model){
        // import org.springframework.ui.Model; -> 뷰(우리가 보는 화면)로 데이터를 넘겨주는 모델 객체
...
/*
    ...
    thymeleafExample() 이라는 메서드는 "example" 이라는 리턴값을 가지는데, 얘가 @Controller 라는 애너테이션과
    합쳐지면 -> 뷰의 이름이라는 의미가 됩니다.
    ...
    모델 역할 살펴 보면
        ...
        모델은 뷰로 해당 데이터를 전달해서 키에 맞는 데이터를 뷰에서 조회할 수 있게 만들어 줍니다.
...
컨트롤러(Controller)---------------------->          hobbies : ["운동", "독서"] -------------------> 뷰(View)
    ...
    다 하신 분들은 src/main/resources/templates 디렉토리에 example.html 생성
